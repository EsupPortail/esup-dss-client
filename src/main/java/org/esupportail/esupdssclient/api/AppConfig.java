...
/**
 * Configuration of the Esup-DSS-Client Platform
 *
...
public class AppConfig {
    ...
    public File getApiHome() {
        ...
        try {
            this.apiHome = configurationManager.manageConfiguration(this.getApplicationName());
        } catch (final IOException e) {
            logger.error("Error while managing config : {}", e.getMessage(), e);
            this.apiHome = null;
        }
        return this.apiHome;
    }
    ...
    public void loadFromProperties(final Properties props) {
        this.setApplicationName(props.getProperty(APPLICATION_NAME, "Esup-DSS-Client"));
                ...
                if (StringUtils.isEmpty(osProperty) || StringUtils.isEmpty((String) entry.getValue())) {
                    ...
                    if (environmentInfo.getOs().equals(osEnum)) {
                    	try {
                    		...
                    		this.defaultProduct = defaultProduct.getDeclaredConstructor().newInstance();
                    	} catch (final ClassNotFoundException | ClassCastException | NoSuchMethodException |
