...
public class KeySelectionController extends AbstractUIOperationController<DSSPrivateKeyEntry> implements Initializable {
    ...
    private List<ImageView> getQCIcons(final CertificateToken certificateToken) throws IOException {
        ...
        final List<String> qcStatements = new ArrayList<String>();
        final byte[] qcStatement = certificateToken.getCertificate().getExtensionValue(Extension.qCStatements.getId());
        if (qcStatement != null) {
            final ASN1Sequence seq = getAsn1SequenceFromDerOctetString(qcStatement);
            // Sequence of QCStatement
            for (int ii = 0; ii < seq.size(); ii++) {
                qcStatements.add(new String(seq.getObjectAt(ii).toASN1Primitive().getEncoded()));
            }
        }
